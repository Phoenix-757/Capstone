...
def combine_packets(data):
    # USE .index(substring, start, finish) -> string.index("][") returns index of "][" 
    # .find(string) -> -1 not found, 1 found
    packet_finished = False
    if data.find("[") == 1 and data.find("]") == -1: # found start, but not end
        # Copy all data after [ for evaluation
        firstIndex = data.index("[")
        data = data[firstIndex:len(data)]
        file_over = open("testingText.txt","w") # w -> overwrite
        file_over.write(data)
        file_over.close()
    elif data.find("[") == -1 and data.find("]") == 1: # found end, but not start
        # Copy all data before ] for evaluation 
        last_index = data.index("]")
        data = data[0:last_index]
        file_append = open("testingText.txt","a") # a -> append
        file_append.write(data)
        file_append.close()
        packet_finished = True
    elif data.find("[") == 1 and data.find("]") == 1: # found start and end
        # Copy all data in between [] for evaluation
        firstIndex = data.index("[")
        lastIndex = data.index("]")
        data = data[firstIndex:lastIndex]
        file_over = open("testingText.txt","w")
        file_over.write(data)
        file_over.close()
        packet_finished = True
    else: # found neither start nor end
        # Copy all data for evaluation
        file_append = open("testingText.txt","a")
        file_append.write(data)
        file_append.close()
